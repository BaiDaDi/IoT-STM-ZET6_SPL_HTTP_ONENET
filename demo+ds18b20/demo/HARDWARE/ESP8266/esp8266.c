...
char *str[4] = {"POST /devices/1061124568/datapoints HTTP/1.1",
				"api-key:dNnclKLT=R9q=MHsV5A8xedxJ6M=",
...
u16 esp8266_get_data(char *vStr)
{
	...
	u16 value = 0;
	char *back;
	char temp[160] = "GET /devices/1061124568/datastreams/";
	...
	//printf("GET server = %s \r\n", back);//PT:观察get是否成功，会返回长段落，可以直接看数值的
...
void esp8266_start_trans(void)
{
	...
	LCD_ShowString(30,210,200,16,16,"WIFI?");
	//让模块连接上自己的路由WIFI GOT IP
	while (esp8266_send_cmd("AT+CWJAP=\"MasterC\",\"66669999\"", "WIFI GOT IP", 500)){
		delay_ms(2);
	};
	...
	LCD_ShowString(30,210,200,16,16,"TCP?");
	...
	//while(esp8266_send_cmd("*582341#123456#test*","receivedreceived",200));
